#Para Git
# Define o Path onde o arquivo de saida para rollback sera savo
$outputFilePath = "$PSScriptRoot\IE_Original-Policy-Status.txt"

# Define uma lista com propriedades de atributos de seguranca
$policies = @(
...
# Define a lista com o valores sugeridos pelo BitDefender GavityZone
$NewPolicies = @(
...
# Step for Debug: Verifica o status atual de cada policy e salva os resultados em um arquivo.
$policyStatuses = @()
...
# Step for Backup: Realiza o backup do status atual das policies settings
function backupCurrentPolicies {
    $backupFile = "IE_Policy-Backup_$(Get-Date -Format 'ddMMyyyy_HHmmss').txt"
...
function applyNewPolicies {
    $npsAppliedFile = "IE_NewPoliciesSetApplied_$(Get-Date -Format 'ddMMyyyy_HHmmss').txt"
    foreach ($npolicy in $NewPolicies) {
        ...
        #Write-Host "Nova policy setting: " $npolicy.NameAction " - Aplicada"
    }
    Write-Host "Aplicacao dos novos valores de policies settings finalizada. Arquivo gerado: $npsAppliedFile"
    $global:newPolicyApplied = $npsAppliedFile
}

function rollbackPolicies {
    $rollbackData = Get-Content -Path $backupFileName
